...
session_start();
   ...
   if(isset($_POST['ac']) && isset($_POST['pw'])){
    ...
    while ($row = $stmt -> fetch(PDO::FETCH_ASSOC)) {
      if($row['fid']==$_POST['ac'] && $row['pass']==$_POST['pw'] ){
        session_start();
        $_SESSION['name']=$row['fid'];
      }     
 }
if($_SESSION['name']==$_POST['ac'])
{
  $_POST=array();
  header("Location: fdash.html");
}
}
...
<body class="text-center">
	<form class="form-signin" method="post">
     <!-- <img class="mb-4" src="../images/logo0.png" alt="adminlogin" width="72" height="72"> -->
      <h1 class="h3 mb-3 font-weight-normal">Please sign in Sir/Mam</h1>
      <label for="inputEmail" class="sr-only">FID</label>
      <input type="text" id="inputEmail" class="form-control" name="ac" placeholder="FID" required autofocus>
